	...
	Qrcode.prototype = {
		...
		weiBoBridge: function () {
			window.WeiboJSBridge.invoke('scanQRCode', null, function (params) {
				...
				if (params.result.indexOf("error")>=0) {
					mui.alert('二维码未识别', '警告');
					return;
				}
				var htm = "<li class='mui-table-view-cell'>";
				htm += params.result;
				htm += "<button type='button'' class='mui-btn mui-btn-primary'>删除</button>";
				htm += "</li>";
				$('.result-qrcode').append(htm);
				mui.alert('识别成功', '警告');
			});
		},
		getImgFile: function () {
